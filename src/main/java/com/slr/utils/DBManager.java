...
public class DBManager {
	...
	public void startHSQLDB() {
		...
		hsqlServer.start();
		LogManager.LOGGER.info("HSQL Server started");
	}
	...
	public void stopHSQLDB() {
		...
		if (connection != null) {
			try {
				connection.close();
				LogManager.LOGGER.info("Connection to HSQL Server closed");
		...
		hsqlServer.stop();
		LogManager.LOGGER.info("HSQL Server stopped");
	}
	...
	public void createHSQLDBTable() {
		...
		try {
			...
			if (!tables.next()) {
				...
				LogManager.LOGGER.fine("Table "+TABLE_NAME+" created");
			}
	...
	public void insertEvents(List<LogEvent> eventList) {
		...
		try {
			...
			for (LogEvent e : eventList) {
				...
				if (!existingRow.next()) {
					...
					LogManager.LOGGER.fine("Row not inserted row as ID "+e.getId()+ " already exists");
				}
		...
		LogManager.LOGGER.fine(insertedRows+" rows inserted in the db");
	}
